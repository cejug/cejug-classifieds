...
public class CRUDEntityFacade<T extends AbstractEntity> implements EntityFacade<T> {
    ...
    public List<T> readAll(Class<T> entityClass) throws Exception {
        ...
        Query query = manager.createQuery("select e from " + entityClass.getSimpleName() + " e");
     ...
     * @param primaryKey The ID of the entity. Cejug-Classifieds supports
     *            Integer IDs only.
     * @return entity The entity found in the database.
     * @throws Exception if the database does not contains the entity that
     *             matches the ID provided as parameter.
     */
    @Override
    public T read(Class<T> entityClass, Serializable primaryKey) throws Exception {
     ...
     */
    @Override
    public void delete(Class<T> entityClass, Serializable primaryKey) throws Exception {
