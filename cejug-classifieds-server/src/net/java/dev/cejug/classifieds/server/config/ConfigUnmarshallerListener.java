...
public class ConfigUnmarshallerListener extends Listener {
   ...
   * 
   * @param logger the logger used to register the listener processing.
   */
  ...
  public ConfigUnmarshallerListener() {
    this(LogManager.getLogManager().getLogger(ConfigUnmarshallerListener.class.getName()));
  }
  ...
  private void validateServiceImplementation(final Injection serviceImpl) {
     ...
     * (ClassNotFoundException e) { logger.severe(String.format( "The service
     * implementation cannot be found: {0}", implementation)); throw new
     * WebServiceException(e); } catch (InstantiationException e) { logger
     * .severe(String .format( "The service implementation cannot be
     * instantiated: {0}", e.getMessage())); throw new WebServiceException(e); }
     * catch (IllegalAccessException e) { logger .severe(String .format(
